...
@onready var broken_time: Timer = $brokenTime
@onready var reset_time: Timer = $resetTime
...
func _on_broken_time_timeout() -> void:
	...
	fade_out.tween_property(anim, "modulate:a", 0, 0.5)
	reset_time.start()

func _on_reset_time_timeout() -> void:
	is_broken = false
	anim.play("default")
	collision_layer = 1
