...
func play_track(world_name: String):
	...
	# SÓ salva se for música de MAPA. Se for boss, mantém o que tava antes.
	if world_name != "boss" and world_name != "extra_life":
...
func _crossfade(new_stream: AudioStream, target_vol: float):
	...
	tween.tween_property(current_p, "volume_db", -80.0, 1.2).set_trans(Tween.TRANS_SINE)
	tween.tween_property(next_p, "volume_db", target_vol, 1.2).set_trans(Tween.TRANS_SINE)
